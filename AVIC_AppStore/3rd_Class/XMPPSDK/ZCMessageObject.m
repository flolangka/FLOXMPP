...
- (instancetype)initWithFrom:(NSString *)from to:(NSString *)to content:(NSString *)body time:(NSDate *)time chatType:(NSString *)chatType
{
    ...
    if (self) {
        ...
        self.messageTo = to;
        self.messageContent = body;
        self.messageDate = time;
...
+(BOOL)save:(ZCMessageObject*)aMessage
{
    ...
    NSString *createStr=@"create table zcMessage (messageFrom,messageTo,messageContent,messageDate,chatType)";
    ...
    if (!isData) {
        //如果表内没有数据，就插入数据
      isData=  [db executeUpdate:insertStr,aMessage.messageFrom,aMessage.messageTo,aMessage.messageContent,aMessage.messageDate,aMessage.chatType];
    }else{
        //如果表内有数据，就更新数据
...
+(NSMutableArray *)fetchRecentChatByPage:(int)pageIndex
{
    ...
    while ([rs next]) {
        ...
        if (messageSQL) {
            ...
            [messageSQL addObject:[rs stringForColumn:@"chatType"]];
...
// NSDictionary*dic=@{@"des":str,@"num":str2,@"time":str3,@"from":str4};
    ...
    [db executeUpdate:upDateStr,[NSString stringWithFormat:@"%@%@",ChatType_Group,dic[@"des"]],user,user];
    [db close];
